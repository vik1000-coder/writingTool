...
test("LaTeX parser retains offsets, nested headings, starred sections and ignores comments/verbatim", () => {
  ...
  assert.deepEqual(
    ast.headings.map((h) => h.title),
    ["Results with \\emph{care}", "Cost"],
  );
  assert.deepEqual(
    ast.citations.map((c) => c.key),
    ["smith2024", "jones2025"],
  );
  assert.equal(ast.labels[0].name, "sec:results");
  assert.equal(
    text.slice(ast.headings[0].start, ast.headings[0].end),
    "\\section[Short]{Results with \\emph{care}}",
  );
...
test("cursor at paragraph boundary and after trailing newline remains bounded", () => {
  ...
  assert.match(
    cursorContext(text, text.length, parseLatex("x.tex", text)).paragraph,
    /Beta/,
  );
...
test("multi-file expansion follows TeX root lookup, reports missing files, avoids cycles and traversal", () => {
  const files = new Map([
    ["paper/main.tex", "\\input{parts/result}\\include{absent}"],
    [
      "paper/parts/result.tex",
      "\\input{main}\\section{Results}\\input{../../outside}",
    ],
  ]);
