...
### Workspace bibliography and PDFs
...
The Zotero desktop app can supply reference metadata and local PDF attachments without moving the originals into your research folder:

1. Open Zotero. In **Settings → Advanced**, enable **Allow other applications on this computer to communicate with Zotero**.
2. In Research Copilot's **References** panel, click **Connect Zotero**, or run **Connect or Change Zotero Library** from the command palette.
3. Choose **My Library** or a group library, then choose the entire library or one collection. A collection is recommended for large libraries.
4. Research Copilot reads the local Zotero API, imports reference metadata, and copies eligible PDF attachments into its ignored local cache. It does not need a Zotero cloud API key and does not modify Zotero.
5. Search, pin, inspect, and cite imported entries like workspace references. **Open item in Zotero** selects the original item in the desktop app; **Open highlighted passage** uses the derived local PDF copy.
6. Use **Refresh Zotero Library** after Zotero metadata or attachments change. **Disconnect Zotero Library** removes the selected scope, imported artifacts, associated controls, and derived PDF cache; it never deletes Zotero's originals.

The connected scope is limited to 500 bibliographic items, 20 attachments per item, and 250 imported PDFs of at most 20 MiB each. Notes, annotations, and standalone attachments are not imported as bibliography records. A refresh that cannot reach a closed Zotero app leaves the last successful local evidence available and shows a warning; an explicit Zotero refresh reports the connection error.

Citation keys use Zotero's available citation key, its BibTeX export key, or the eight-character Zotero item key as a fallback. A workspace `.bib` entry with the same key remains authoritative. The connection does not create or rewrite a project `.bib` file: for compilable LaTeX citations, keep Zotero or Better BibTeX exporting the same keys into a bibliography used by your manuscript.

There is no DOI lookup, paper download service, Zotero write-back, or dedicated bibliographic metadata editor.
...
| Connect or Change Zotero Library | Choose a local Zotero personal/group library and optional collection, then import read-only evidence. |
| Refresh Zotero Library | Re-read the connected scope and update its derived PDF cache. |
| Disconnect Zotero Library | Remove the connected scope, imported evidence, and derived cache without changing Zotero. |
...
| Show or Generate Ghost Text | Switch to WRITE and show or request a native inline continuation. |
| Suggest for Highlighted Text | Apply the current assistance mode to the selected passage. |
