...
(() => {
  ...
  const descriptions = {
    ...
    structure: "See what the argument still needs.",
  };
  ...
  function empty(title, text) {
    ...
    el.append(node("h2", title), node("p", text, "muted"));
    return el;
  }
  ...
  function source(a) {
    ...
    if (a.kind === "pdf")
      card.append(
        node("blockquote", a.text),
        node("small", "Exact locally extracted source text"),
      );
    else if (a.kind === "result") {
      if (a.metadata.dimensions)
        card.append(
          node(
            "p",
            `${a.metadata.dimensions.rows} rows · ${a.metadata.dimensions.columns} columns`,
            "muted",
          ),
        );
      ...
      if (rows.length) {
        ...
        header.append(node("th", "Row"));
        columns.forEach((c) => header.append(node("th", c)));
        table.append(header);
        rows.slice(0, 12).forEach((row, i) => {
          const tr = node("tr");
          tr.append(node("td", String(a.locator.rows?.[i] || i + 1)));
          columns.forEach((c) => tr.append(node("td", String(row[c] ?? "—"))));
          table.append(tr);
        });
        wrap.append(table);
        card.append(wrap);
      }
      if (a.metadata.statistics) {
        const d = node("details");
        d.append(
          node("summary", "Schema and descriptive statistics"),
          node(
            "pre",
            JSON.stringify(
              { schema: a.metadata.schema, statistics: a.metadata.statistics },
              null,
              2,
            ),
          ),
      ...
      card.append(
        node("small", "Rows count data records, excluding the header."),
      );
    } else if (a.kind === "bib") {
      card.append(
        node(
          ...
          [
            a.metadata.author,
            a.metadata.year,
            a.metadata.journal || a.metadata.booktitle,
          ]
            .filter(Boolean)
            .join(" · "),
          "muted",
      ...
      if (!a.metadata.pdf_path)
        card.append(
          warning("Citation candidate — full-text evidence not verified."),
        );
    } else if (a.kind === "figure" || a.kind === "table") {
      if (a.metadata.caption) card.append(node("p", a.metadata.caption));
      if (a.metadata.label)
        card.append(node("p", a.metadata.label, "source-path"));
      if (a.metadata.first_reference)
        card.append(
          node(
            "p",
            `First reference: ${a.metadata.first_reference.path} · line ${a.metadata.first_reference.line}`,
            "muted",
          ),
        );
      for (const id of a.metadata.source_data || [])
        card.append(button("Data · " + id, () => send("open", { id })));
      for (const id of a.metadata.generated_by || [])
        card.append(button("Code · " + id, () => send("open", { id })));
      if (a.metadata.generated_by?.length)
        card.append(
          node(
            "p",
            "Lineage inferred from code paths. Confirm important relationships in Context.",
            "muted",
          ),
        );
    } else {
      const d = node("details");
      d.append(node("summary", "Inspect artifact"), node("pre", a.text));
      card.append(d);
    }
  ...
  function renderPanel() {
    ...
    if (tab === "Suggestion") {
      if (!suggestion)
        panel.append(
          empty(
            ...
            state.mode === "off"
              ? "Switch modes when you want assistance, or ask in Chat."
              : "Place your cursor in the manuscript, choose a mode, and request a suggestion. Nothing is sent until you ask.",
          ),
        );
      else {
        const card = node("article", undefined, "card primary");
        card.append(
          node("span", suggestion.mode.toUpperCase(), "tag"),
        ...
        if (suggestion.insert_text) {
          card.append(
            ...
            node(
              "small",
              resolved.insertable
                ? "Ghost text is available in the editor. Tab accepts it."
                : "Insertion blocked until evidence issues are resolved.",
            ),
          );
        }
        if (suggestion.proposal) {
          ...
          card.append(
            ...
            node("p", p.purpose),
            node("p", `Placement: ${p.placement}`, "muted"),
          );
          p.panels.forEach((text) => card.append(node("p", text)));
        }
        if (suggestion.edit)
          card.append(
            button(
        ...
        if (resolved.evidence.length)
          panel.append(
            button(
              `Inspect ${resolved.evidence.length} evidence sources →`,
              () => selectTab("Evidence"),
            ),
          );
      }
      ...
      panel.append(
        node(
          "p",
          "Source passages and data are resolved locally, never quoted by the model.",
          "muted",
        ),
      );
      if (!evidence.length)
      ...
      panel.append(
        node("h2", "You choose the context."),
        node(
          "p",
          "Pins take priority. Exclusions remove retrieved artifacts. The manuscript at your cursor is always included; unsaved text overrides saved text.",
          "muted",
        ),
      );
      panel.append(
        button("Inspect exact request & response", () => send("inspect")),
        button("Confirm evidence relationship", () => send("relation")),
        button("Set section goal", () => send("sectionMemory")),
      );
      if (state.context) {
        panel.append(
          node(
            "p",
            `${JSON.stringify(state.context).length.toLocaleString()} context characters · ${state.context.artifacts.length} artifacts`,
            "muted",
          ),
        );
        ...
        panel.append(
          empty(
            ...
            "You can pin local artifacts from the other panels before making a request.",
          ),
        );
      if (state.excluded?.length) {
        panel.append(node("h3", "Excluded from context"));
        state.excluded.forEach((id) =>
          panel.append(button(`Include ${id}`, () => send("exclude", { id }))),
        );
      }
      if (state.relations?.length) {
        const graph = node("details");
        graph.append(
          node("summary", `${state.relations.length} project relationships`),
        );
        state.relations.slice(0, 100).forEach((edge) => {
          const row = node("div", undefined, "card");
          row.append(
            node("span", edge.confirmed ? "CONFIRMED" : "INFERRED", "tag"),
            node(
              "p",
              `${edge.source}\n↓ ${edge.relation}\n${edge.target}`,
              "prose source-path",
      ...
      panel.append(
        node(
          "p",
          "Explicit questions work in every mode, including OFF. Edits require a reviewed diff and your approval.",
          "muted",
        ),
      );
      state.history?.forEach((m) => {
        const el = node("div", undefined, "chat-message");
        el.append(
          node("span", m.role === "user" ? "YOU" : "RESEARCH COPILOT", "tag"),
          node("p", m.text, "prose"),
        );
        panel.append(el);
      });
      const form = node("form"),
        input = node("textarea");
      input.placeholder = "Ask about the paper or request a specific edit…";
      input.setAttribute("aria-label", "Question for Research Copilot");
      input.value = draft;
      input.addEventListener("input", () => {
        draft = input.value;
        vscode.setState({ tab, draft });
      });
      ...
      form.addEventListener("submit", (e) => {
        e.preventDefault();
        if (input.value.trim()) {
          send("chat", { question: input.value.trim() });
          draft = "";
          input.value = "";
          vscode.setState({ tab, draft });
        }
      ...
      if (suggestion?.edit)
        panel.append(
          button("Review proposed diff", () => send("reviewEdit"), "secondary"),
        );
    } else {
      const map = {
        Outline: ["outline"],
      ...
      input.addEventListener("input", () => {
        clearTimeout(timer);
        timer = setTimeout(
          () => send("search", { query: input.value, kinds: map[tab] }),
          250,
        );
      });
      panel.append(input);
      if (tab === "Outline" && state.ephemeralOutline)
        panel.append(
          node(
            "p",
            "From manuscript headings · no outline file was created.",
            "muted",
          ),
        );
      ...
      if (!values.length)
        panel.append(
          empty(
            ...
            "Add project files or adjust .research-copilot/project.yaml, then refresh.",
          ),
        );
    ...
    if (state.indexWarnings?.length) {
      const d = node("details");
      d.append(
        node("summary", `${state.indexWarnings.length} indexing notice(s)`),
      );
      state.indexWarnings.forEach((w) => d.append(warning(w)));
      panel.append(d);
    }
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    const restoreChat = focused?.tagName === "TEXTAREA";
    const restoreSearch = focused?.classList.contains("search");
    const searchValue = restoreSearch ? focused.value : "";
    const start = focused?.selectionStart;
    renderPanel();
    const restore = restoreChat
      ? document.querySelector("textarea")
      : restoreSearch
        ? document.querySelector(".search")
        : null;
    if (restore) {
      if (restoreSearch) restore.value = searchValue;
      restore.focus();
      restore.setSelectionRange(start, start);
    }
  });
  renderPanel();
  send("ready");
