...
export function sidebarHtml(webview: vscode.Webview, extension: vscode.Uri): string {
  const nonce = randomBytes(18).toString('base64');
  const script = webview.asWebviewUri(vscode.Uri.joinPath(extension, 'media', 'sidebar.js'));
  const style = webview.asWebviewUri(vscode.Uri.joinPath(extension, 'media', 'sidebar.css'));
  ...
  <header><div class="eyebrow">RESEARCH COPILOT</div><div class="heading-row"><h1>Think, then write.</h1><button id="setup" class="quiet" title="Check local setup" aria-label="Check local setup">⚙</button></div><label class="mode-label" for="mode">ASSISTANCE MODE</label><select id="mode"><option value="off">OFF · Observe</option><option value="guide" selected>GUIDE · Next idea</option><option value="write">WRITE · Continue</option><option value="evidence">EVIDENCE · Verify</option><option value="visual">FIGURE / TABLE · Present</option><option value="structure">STRUCTURE · Organize</option></select><p id="mode-help" class="muted"></p><div class="actions"><button id="suggest">Suggest next step</button><button id="cancel" class="secondary" hidden>Cancel</button></div><p id="status" class="status" role="status" aria-live="polite">Open a LaTeX research project to begin.</p></header>
