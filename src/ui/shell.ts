...
export function sidebarHtml(
  ...
  <footer><div class="usage" role="status" aria-live="polite" title="xAI-reported cost when available; otherwise an estimate using documented model token rates."><strong id="usage-label">No model usage this session</strong><span id="usage-detail">Token and price counts appear after a provider response.</span></div><span id="project-label">Local project · Read-only suggestions</span><button id="refresh" class="link">Refresh index</button></footer><script nonce="${nonce}" src="${script}"></script></body></html>`;
}
