...
export const OUTPUT_SCHEMA = object({
  mode: {
    type: "string",
    enum: ["guide", "write", "evidence", "visual", "structure", "chat"],
  },
  title: string,
  text: string,
  insert_text: string,
  ...
  proposal: {
    anyOf: [
      object({
        type: { type: "string", enum: ["figure", "table", "none"] },
        purpose: string,
        placement: string,
        data_ids: strings,
        existing_artifact_ids: strings,
        panels: strings,
      }),
      { type: "null" },
    ],
  ...
  edit: {
    anyOf: [
      object({ path: string, original: string, replacement: string }),
      { type: "null" },
    ],
  },
});
...
export function validateSuggestion(
  ...
  if (
    ...
    throw new Error("Model response mode does not match request");
  const insert = text(r.insert_text, 600);
  if (
    insert &&
    (/\\(?:input|include|write|openout|read|catcode|def|edef|gdef|xdef|newcommand|renewcommand|usepackage|documentclass|csname|special)(?![A-Za-z])/.test(
      insert,
    ) ||
      /(?<!\\)(?:\\\\)*%/.test(insert))
  )
    throw new Error(
      "WRITE must contain prose, not TeX execution directives or unescaped comments",
    );
  if (mode !== "write" && insert)
    throw new Error("Only WRITE can provide insertable prose");
  if (
    insert &&
    (/\n\s*\n/.test(insert) ||
      (insert.match(/[.!?](?:\s|$)/g)?.length ?? 0) > 2)
  )
    throw new Error("WRITE is limited to a clause or one to two sentences");
  if (!Array.isArray(r.claims) || r.claims.length > 30)
    throw new Error("Invalid numeric claims");
  const claims = r.claims.map((v) => {
    const c = record(v);
    if (c.row !== null && (!Number.isInteger(c.row) || Number(c.row) < 1))
      throw new Error("Invalid result row");
    return {
      value: text(c.value, 100),
      artifact_id: text(c.artifact_id, 1000),
      source_hash: text(c.source_hash, 100),
      row: c.row as number | null,
      column: c.column === null ? null : text(c.column, 500),
    };
  });
  let proposal: Suggestion["proposal"] = null;
  if (r.proposal != null) {
    const p = record(r.proposal);
    if (!["figure", "table", "none"].includes(String(p.type)))
      throw new Error("Invalid visualization type");
    proposal = {
      type: p.type as "figure" | "table" | "none",
      purpose: text(p.purpose),
      placement: text(p.placement),
      data_ids: list(p.data_ids),
      existing_artifact_ids: list(p.existing_artifact_ids),
  ...
  if (r.edit != null) {
    if (mode !== "chat")
      throw new Error("Only explicit Chat can propose an edit");
    const e = record(r.edit);
    edit = {
      path: text(e.path, 1000),
      original: text(e.original),
  ...
  const title =
    text(r.title, 200) ||
    {
      guide: "Next sentence intention",
      write: "Continue your thought",
      evidence: "Evidence assessment",
      visual: "Figure / table proposal",
...
export function numericTokens(value: string): string[] {
  const stripped = value.replace(
    /\\(?:[a-zA-Z]*cite[a-zA-Z]*|label|ref|eqref|autoref|cref|Cref)\*?(?:\[[^\]]*\])*\{[^}]*\}/g,
    "",
  );
  return [
    ...stripped.matchAll(
      /(?<![\p{L}\p{N}_.])[-+−]?(?:\d+(?:,\d{3})*(?:\.\d+)?|\.\d+)(?:[eE][-+−]?\d+)?/gu,
    ),
  ].map((m) => m[0]);
}
const numericValue = (s: unknown) =>
  typeof s === "number"
    ? s
    : typeof s === "string" &&
        /^[-+−]?(?:\d[\d,]*(?:\.\d+)?|\.\d+)(?:[eE][-+−]?\d+)?$/.test(s.trim())
      ? Number(s.replaceAll(",", "").replaceAll("−", "-"))
      : NaN;
function grounded(c: NumericClaim, artifacts: Map<string, Artifact>): boolean {
  ...
  if (
    !a ||
    a.kind !== "result" ||
    c.source_hash !== a.hash ||
    c.row === null ||
    c.column === null ||
    !a.locator.columns?.includes(c.column)
  )
    return false;
...
export function resolveSuggestion(
  ...
  const ids = [
    ...new Set([
      ...
      ...suggestion.outline_ids,
      ...(suggestion.proposal?.data_ids ?? []),
      ...(suggestion.proposal?.existing_artifact_ids ?? []),
    ]),
  ];
  const evidence = ids.flatMap((id) => {
    const a = lookup.get(id);
    if (!a) {
      warnings.push(`Unknown or stale evidence: ${id}`);
      return [];
    }
    return [a];
  });
  const allProse = [
    ...
    suggestion.proposal?.purpose ?? "",
    ...(suggestion.proposal?.panels ?? []),
  ].join("\n");
  const citeKeys = new Set([
    ...suggestion.citation_keys,
    ...parseLatex("", allProse).citations.map((c) => c.key),
  ]);
  let invalidCitation = false;
  for (const key of citeKeys) {
    const bib = lookup.get(`bib:${key}`);
    if (!bib || bib.kind !== "bib") {
      warnings.push(`Unknown citation key: ${key}`);
      invalidCitation = true;
    } else if (
      !artifacts.some(
        (a) =>
          a.kind === "pdf" &&
          (a.metadata.citation_key === key || a.path === bib.metadata.pdf_path),
      )
    )
      warnings.push(
        `${key}: Citation candidate — full-text evidence not verified.`,
      );
  }
  const validClaims = suggestion.claims.filter((c) => grounded(c, lookup));
  for (const c of suggestion.claims)
    if (!validClaims.includes(c))
      warnings.push(
        `UNGROUNDED value ${c.value}: result locator is missing, mismatched, or stale.`,
      );
  const grounding = [...new Set(numericTokens(allProse))].map((value) => {
    const locator = validClaims.find(
      (c) => numericValue(c.value) === numericValue(value),
    );
    if (!locator)
      warnings.push(
        `UNGROUNDED value ${value}: empirical value not verified against indexed results.`,
      );
    return {
      value,
      status: locator ? ("GROUNDED" as const) : ("UNGROUNDED" as const),
      ...(locator ? { locator } : {}),
    };
  });
  ...
  const insertable =
    ...
    Boolean(suggestion.insert_text) &&
    !invalidCitation &&
    !ids.some((id) => !lookup.has(id)) &&
    insertedNumbers.every((value) =>
      validClaims.some((c) => numericValue(c.value) === numericValue(value)),
    );
  return {
    suggestion,
    evidence,
