...
export class JsonLineClient extends EventEmitter {
  ...
  constructor(
    ...
    this.child.stderr.on("data", (chunk: string) =>
      options.log?.(chunk.slice(0, 4000)),
    );
    ...
    this.child.on("exit", (code, signal) =>
      this.fail(
        new Error(
          `Helper exited (${signal ?? code}). Check executable and dependencies.`,
        ),
      ),
    );
  }
  private read(chunk: string) {
    ...
    if (this.buffer.length > 32 * 1024 * 1024) {
      this.fail(new Error("Helper response exceeds size limit"));
      this.child.kill();
      return;
    }
    let at;
    while ((at = this.buffer.indexOf("\n")) >= 0) {
      ...
      try {
        ...
        this.fail(new Error("Invalid JSON from local helper"));
        this.child.kill();
        return;
      }
      if (message.method) {
